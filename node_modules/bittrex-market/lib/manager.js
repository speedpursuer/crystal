...
class MarketManager extends EventEmitter {
    ...
    _addClient(callback) {
        ...
        client.serviceHandlers.connectFailed = (websocket) => {
            console.log('connectFailed', this.index)
            this.emit('connectFailed', client)
        }

        client.serviceHandlers.connectionLost = (websocket) => {
            console.log('connectionLost', this.index)
        ...
        client.on('CoreHub', 'updateExchangeState', (message) => {
            if(this._markets[message.MarketName]) {
        ...
        console.log(`当前signalr client数量: ${this._clientPool.length}`)
    }
