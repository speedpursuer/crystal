...
module.exports = class zb extends Exchange {
    ...
    async fetchBalance (params = {}) {
        ...
        let currencies = balances['coins'];

        for (let i = 0; i < currencies.length; i++) {
            let currency = currencies[i];
            let account = this.account ();
            account['free'] = parseFloat (currency['available']);
            account['used'] = parseFloat (currency['freez']);
            account['total'] = this.sum (account['free'], account['used']);
            result[currency['enName']] = account
        }
    ...
    async fetchOpenOrders (symbol = undefined, params = {}) {
        await this.loadMarkets ();
        try{
            return await this.privatePostGetUnfinishedOrdersIgnoreTradeType ({
                'currency': this.marketId (symbol)
            });
        }catch (e) {
            if(e.message.includes('3001')) {
                return []
            }else {
                throw e
            }
    ...
    sign (path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api == 'public') {
            ...
            let auth = this.urlencode (this.keysort (
                this.extend ({
                    'method': path,
