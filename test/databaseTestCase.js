...
const redis = require('../service/redisDB.js')
...
describe('测试Database class', async function() {
	...
	var curreData
    var mongo = new MongoDB('localhost', 'test')

	before(async function() {		
		await redis.initAccount('database test case', 1000, 1, ['k', 'p'])
	})
	...
	describe.only('recordBalance', async function() {
  	    it('交易结果不应该为0', async function() {
            await redis.recordBalance(50, -1)
            curreData = await redis.getData()
  ...
  describe.only('getOrderBooks', async function() {
      it('获取时间条数', async function() {
          ...
          result = await mongo.getOrderBooks('BTC/USD', ['bitstamp', 'lakebtc', 'bitfinex'], 0,
              util.timestamp)
        util.log("got data")
      })
  })
